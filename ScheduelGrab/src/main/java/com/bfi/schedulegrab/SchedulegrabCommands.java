...
@Service
public class SchedulegrabCommands implements CommandMarker { // All command types must implement the CommandMarker interface
    ...
    /**
     * Get a reference to the SchedulegrabOperations from the underlying OSGi container
     */
    @Reference private SchedulegrabOperations operations; 
    ...
    protected void activate(ComponentContext context) {
        staticFieldConverter.add(SchedulegrabPropertyName.class);
    }
    ...
    protected void deactivate(ComponentContext context) {
        staticFieldConverter.remove(SchedulegrabPropertyName.class);
    }
    ...
    public void sayHello(
        ...
        @CliOption(key = "countryOfOrigin", mandatory = false, help = "Country of orgin") SchedulegrabPropertyName country) {
        
        log.info("Welcome " + name + "!");
        log.warning("Country of origin: " + (country == null ? SchedulegrabPropertyName.NOT_SPECIFIED.getPropertyName() : country.getPropertyName()));
        log.info("It seems you are a running JDK " + operations.getProperty("java.version"));
