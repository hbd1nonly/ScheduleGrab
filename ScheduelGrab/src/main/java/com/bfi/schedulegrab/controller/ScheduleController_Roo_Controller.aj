...
import com.bfi.schedulegrab.controller.ScheduleController;
...
privileged aspect ScheduleController_Roo_Controller {
    ...
    @Autowired
    ScheduleService ScheduleController.scheduleService;
    ...
    public String ScheduleController.show(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("schedule", scheduleService.findSchedule(id));
    ...
    public String ScheduleController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("schedules", scheduleService.findScheduleEntries(firstResult, sizeNo));
            float nrOfPages = (float) scheduleService.countAllSchedules() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("schedules", scheduleService.findAllSchedules());
        }
    ...
    public String ScheduleController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, scheduleService.findSchedule(id));
        return "schedules/update";
    }
